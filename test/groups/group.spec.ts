...
test.group('Groups', (group) => {
  ...
  test('it should return 404 when providing an unexisting group for update', async (assert) => {
    await GroupFactory.merge({ master: user.id }).create()
    const payload = {
      name: 'test',
      description: 'test',
      schedule: 'test',
  ...
  test('it should remove an user from the group', async (assert) => {
    const group = await GroupFactory.merge({ master: user.id }).create()
  ...
  test('it should not remove the master from the group', async (assert) => {
    const groupPayload = {
      name: 'D&D',
      description: 'A table for D&D',
      schedule: 'periodical',
  ...
  test('it should return all groups by user id and name', async (assert) => {
    ...
    const { body } = await supertest(BASE_URL)
      .get(`/groups?user=${user.id}&text=D&`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200)
  ...
  test('it should return all groups by user id and description', async (assert) => {
    ...
    const { body } = await supertest(BASE_URL)
      .get(`/groups?user=${user.id}&text=table`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200)
